...
**Working MVP (already public, already proven on-chain):** github.com/EfeDurmaz16/oz-policy-builder — the full pipeline works end to end today: live Horizon fetcher (tested against mainnet: 200 real payments profiled, policy proposed, 200/200 legitimate transfers pass), IQR-based outlier flagging, and a policy installed and enforced on Stellar testnet against a real OZ smart account using exactly the generated params. On-chain, inclusive-boundary checked: under-cap and exact-cap transfers allowed, over-cap and cap-plus-one-stroop rejected with `SpendingLimitExceeded` (#3221). Transaction hashes and the full reproducible record (including the `__check_auth` signing flow) are in the repo under docs/testnet-proof.
...
**Milestones (sketch, to be detailed in the Build form; milestone 0 is already done pre-funding):**
0. ~~Soroban integration: deploy and exercise against a real OZ smart account on testnet; generated cap accepts observed history, rejects an over-cap drain on-chain.~~ Done, see docs/testnet-proof.
1. Production-grade installer: one-command flow wrapping the validated `add_context_rule` path (custom `__check_auth` signing included), emit hardened for Token-2022-style variants and multi-asset accounts.
2. Policy coverage: threshold and weighted-signer inference, context-rule suggestions from the observed counterparty graph, multi-asset profiles.
